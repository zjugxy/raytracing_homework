...
static float F_schlick(float R0,float costheta){
    return R0+(1-R0)*std::pow(1-costheta,5.0f);
}
...
public:
  ...
  // 根据transparent 计算
    ...
    virtual bool scatter(const Ray &rayin, const hitrecord &rec, vec3 &attenuation, Ray &scatter, float &PDF)
    {
        ...
        float costheta1 = std::abs(glm::dot(glm::normalize(rayin.dir), recnorm));
        float CUT = F_schlick(R0,costheta1);
        // sign < 0  从空气至介质 >0 从介质至空气
        if (randnum > CUT)
        {
            // diffuse
            vec3 dir = glm::normalize(-1.0f*rayin.dir);

            vec3 w_i = glm::reflect(-1.0f*dir,recnorm);
            scatter = Ray(rec.position, w_i);
            attenuation = diffuse;
            PDF = 1;
            return true;
        ...
        else
        {
            ...
            else
            {
                ...
                vec3 w_t = (sign < 0) ? (sintheta2 * (-recnorm) + costheta2 * (dir + sintheta1 * recnorm)) / Ni : (sintheta2 * (-recnorm) + costheta2 * (dir - sintheta1 * recnorm)) * Ni;
